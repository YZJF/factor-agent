...
卖方可验证研报抽取 → 规则门禁 → 两阶段 LoRA（SFT / GRPO）→ 冻结字段后的多轮回测演化。
...
门禁：总分 ≥ 0.7 **且** 未触发任何上限 **且** 表达式可执行。未过的样本留在阶段 A 当负样本，不进厨房。
...
两个 LoRA 都从**同一份冻结的 Qwen3-4B** 长出来，进化适配器不叠在抽取适配器上，避免第二段 GRPO 冲掉第一段。
...
Same CLI as in the Chinese section. Convert jsonl → parquet, then:
